...
class CONSYS:
    ...
    def run_system(self, num_epochs, timesteps):
        ...
        # NOTE: jit makes everything 100x faster 
        #       we also need to specify that the second arg (timesteps) is static (not jax arrays)
        gradfunc = jax.jit(gradfunc, static_argnums=[1])
        mse_history = []
        
        for epoch in range (1,num_epochs):
            self.plant.reset()  # NOTE: this does nothing since "grad_func" is already traced above
            self.controller.error_history = jnp.zeros(timesteps)
            mse, gradients = gradfunc(self.controller.get_params(), timesteps) 

            print(epoch, mse)  # NOTE: printing out progress 
...
class Controller:
    ...
    # Plot the progression of learning
        ...
        # Plot the changes in k parameters across epochs (MÃ¥ bare vÃ¦re for PID)
        plt.figure(figsize=(10, 6))
...
class PIDController(Controller):
    ...
    def run_one_epoch(self, params, timesteps):
        # NOTE: this function is now pure. It doesn't modify anything outside it
        U = 0
        plant = copy.deepcopy(self.plant)  # NOTE: Making deepcopy here so that when we modify plant, we don't modify self.plant
        error_history = jnp.array([])  # NOTE: storing error history in local variable so we don't modify anything outside function
        for t in range(timesteps):
            plant_output = plant.update(U) 
            error = plant.target - plant_output 
            error_history = jnp.append(error_history, error)
            U = self.compute(params, error, error_history, t)
    ...
    def compute(self,params, error, error_history, timestep):
        # NOTE: error_history now passed as an argument 
        integral = jnp.sum(error_history)
        derivative = error-error_history[timestep-1]
        kp = params[0]
        ki = params[1]
        kd = params[2]
        p = kp*error
...
class CournotPlant:
    ...
    def update(self,U): 
        ...
        # NOTE: Clipping with jnp. Using python builtins often breaks tracing
...
# NOTE: Way smaller initial values so that it doesn't blow up
kp = 0.0001
ki = 0.0001
kd = 0.0001

num_epochs = 100
learning_rate = 1e-5  # NOTE: smaller LR 
timesteps = 10
